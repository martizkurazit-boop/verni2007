  ...
  function drawStats(d, err) {
    ...
    box.innerHTML = '<div class="eyebrow"><span class="sl">//</span><span>Метрика · счётчик '
      ...
      + ((d.errors || []).length ? '<div class="note-danger" style="margin-top:24px">'
          ...
          + '</ul>'
          // Этот отказ мы уже разобрали: код и токен ни при чём, тот же запрос
          // к соседнему счётчику проходит. Незачем гонять человека по кругу.
          + (d.errors.some(function (e) { return /слишком сложн|too complicated/i.test(e); })
            ? '<p class="hint" style="margin-top:10px">Так Метрика отвечает на любой отчёт по счётчику '
              + esc(d.counter) + ' — даже на «визиты за вчера». Тот же запрос к другому счётчику '
              + 'проходит, значит дело не в сайте и не в токене, а на стороне Метрики. '
              + 'Сбор данных при этом идёт: цифры видны в кабинете. Стоит написать в поддержку Метрики, '
              + 'приложив номер счётчика и текст ошибки.</p>'
            : '<p class="hint" style="margin-top:10px">Отчёты забираются заново каждые шесть часов. '
              + 'Если ошибка повторяется — запустите «Обновить данные» вручную: '
              + 'Actions → «Обновить данные (Метрика и YouTube)» → Run workflow.</p>')
          + '</div>' : '')
